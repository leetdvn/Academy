...
public class FGDatabaseRef {
	...
	public FGDatabaseRef(String databaseUrl, String path, long callbackAddr) {
	...
	private final ValueEventListener mValueListener = new ValueEventListener() {
		@Override
		public void onDataChange(DataSnapshot dataSnapshot) {
			FGDataSnapshot data = new FGDataSnapshot(dataSnapshot);
			onDataChangedCallback(mCallbackAddr, data);
		}

		@Override
		public void onCancelled(DatabaseError databaseError) {
			onCancelledCallback(mCallbackAddr, databaseError.getCode(), databaseError.getMessage());
		}
	};

	private final ChildEventListener mChildListener = new ChildEventListener() {
		@Override
		public void onChildAdded(DataSnapshot dataSnapshot, String s) {
			FGDataSnapshot data = new FGDataSnapshot(dataSnapshot);
			onChildEventCallback(mCallbackAddr, CHILD_EVENT_ADDED, data, s);
		}

		@Override
		public void onChildChanged(DataSnapshot dataSnapshot, String s) {
			FGDataSnapshot data = new FGDataSnapshot(dataSnapshot);
			onChildEventCallback(mCallbackAddr, CHILD_EVENT_CHANGED, data, s);
		}
		...
		public void onChildMoved(DataSnapshot dataSnapshot, String s) {
			FGDataSnapshot data = new FGDataSnapshot(dataSnapshot);
			onChildEventCallback(mCallbackAddr, CHILD_EVENT_MOVED, data, s);
		}

		@Override
		public void onCancelled(DatabaseError databaseError) {
			onChildCancelledCallback(mCallbackAddr, databaseError.getCode(), databaseError.getMessage());
		}
	};
	...
	public void GetValue() {
		if (mDbRef != null) {
			...
			if (mCurrentValueListener != null) q.removeEventListener(mCurrentValueListener);
			q.addListenerForSingleValueEvent(mValueListener);
		}
	}
	...
	public void AddValueListener() {
		if (mDbRef != null) {
			Query q = mQuery != null ? mQuery : mDbRef;
			if (mCurrentValueListener != null) q.removeEventListener(mCurrentValueListener);
			mCurrentValueListener = q.addValueEventListener(mValueListener);
		}
	}
	...
	public void AddChildListener() {
		if (mDbRef != null) {
			Query q = mQuery != null ? mQuery : mDbRef;
			if (mCurrentChildListener != null) q.removeEventListener(mCurrentChildListener);
			mCurrentChildListener = q.addChildEventListener(mChildListener);
		}
	}
		...
		if (mDbRef != null) {
			mDbRef.runTransaction(new Transaction.Handler() {
				...
				public Transaction.Result doTransaction(MutableData mutableData) {
					...
					int result = doTransactionCallback(mCallbackAddr, data);
				...
				public void onComplete(DatabaseError databaseError, boolean b, DataSnapshot dataSnapshot) {
					...
					onTransactionCompleteCallback(mCallbackAddr, b, errorMessage);
				}
			});
		}
	}
