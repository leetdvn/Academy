...
public:
	UFGDataSnapshot();
	virtual ~UFGDataSnapshot();

#if PLATFORM_ANDROID
	void InitPlatformData(jobject DataSnapshot);
#endif

#if PLATFORM_IOS
	void InitPlatformData(FIRDataSnapshot* DataSnapshot);
#endif

#if (PLATFORM_WINDOWS || PLATFORM_MAC) && FG_ENABLE_EDITOR_SUPPORT
	void InitPlatformData(firebase::database::DataSnapshot DataSnapshot);
#endif
...
private:
	TSharedPtr<DataSnapshotPlatformData> PlatformData;
};
