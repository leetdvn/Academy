...
public:
	...
	*/
	...
	void RunTransaction();
	int RunTransactionHandler(UFGMutableData* Data);
	void TriggerTransactionComplete();
	void TriggerTransactionAbort(const FString& ErrorMessage);

	void OnDataChanged(UFGDataSnapshot* Data) const { OnDataChangedDelegate.ExecuteIfBound(Data); }
	void OnCancelled(int ErrorCode, const FString& ErrorMessage) const { OnCancelledDelegate.ExecuteIfBound(ErrorCode, ErrorMessage); }
	void OnChildEvent(EChileEventType Type, UFGDataSnapshot* Data, const FString& ErrorMessage) const { OnChildEventDelegate.ExecuteIfBound(Type, Data, ErrorMessage); }
	void OnChildCancelled(int ErrorCode, const FString& ErrorMessage) const { OnChildCancelledDelegate.ExecuteIfBound(ErrorCode, ErrorMessage); }

private:
	FString GetPathToParent() const;

	FString RefPath;
	TSharedPtr<DatabaseRefPlatformData> PlatformData;
	...
	FOnCancelledDelegate OnChildCancelledDelegate;
};
